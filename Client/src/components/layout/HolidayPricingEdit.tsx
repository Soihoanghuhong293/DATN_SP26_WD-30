...
import { Form, Input, InputNumber, Button, DatePicker, Select, Card, message, Space, Spin } from 'antd';
...
const HolidayPricingEdit = () => {
  ...
  const holidayPricingUrl = useMemo(() => {
    if (!id) return null;
    return `http://localhost:5000/api/v1/holiday-pricings/${id}`;
  }, [id]);
  ...
  // lấy dữ liệu cấu hình hiện tại để fill form
  useEffect(() => {
    const fetchPricing = async () => {
      if (!holidayPricingUrl) {
        setPageLoading(false);
        message.error('Thiếu id cấu hình ngày lễ.');
        return;
      }

      setPageLoading(true);
      try {
        const res = await axios.get(holidayPricingUrl);
        const pricing = res.data?.data;
        if (!pricing) throw new Error('Không tìm thấy dữ liệu');

        const tourIdValue =
          pricing.tour_id && typeof pricing.tour_id === 'object'
            ? pricing.tour_id._id || pricing.tour_id.id
            : pricing.tour_id || null;

        form.setFieldsValue({
          name: pricing.name,
          tour_id: tourIdValue ?? null,
          dateRange: [dayjs(pricing.start_date), dayjs(pricing.end_date)],
          price_multiplier: pricing.price_multiplier ?? 1.0,
          fixed_price: pricing.fixed_price ?? null,
          priority: pricing.priority ?? 0,
        });
      } catch (error: any) {
        message.error(error.response?.data?.message || 'Có lỗi xảy ra khi tải dữ liệu cấu hình!');
      } finally {
        setPageLoading(false);
      }
    };
    ...
    fetchPricing();
  }, [form, holidayPricingUrl]);

  const onFinish = async (values: any) => {
    if (!holidayPricingUrl) return;
    ...
    try {
      ...
      await axios.put(holidayPricingUrl, payload);
      message.success('Cập nhật cấu hình giá ngày lễ thành công!');
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <Card title="Cập Nhật Cấu Hình Giá Ngày Lễ" style={{ maxWidth: 800, borderRadius: 8 }}>
        {pageLoading ? (
          ...
          >
            ...
            <Form.Item name="tour_id" label="Áp dụng cho Tour (Để trống nếu áp dụng cho TẤT CẢ các tour)">
              <Select
            ...
            <Form.Item style={{ marginTop: 24 }}>
              <Button type="primary" htmlType="submit" icon={<SaveOutlined />} size="large" loading={saving} block>
                Lưu Thay Đổi
              </Button>
            </Form.Item>
          </Form>
...
export default HolidayPricingEdit;
