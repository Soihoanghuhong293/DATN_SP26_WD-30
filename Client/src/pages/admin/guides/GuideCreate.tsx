...
import { Form, Input, Button, message, Card, Select, DatePicker, InputNumber, Upload, Space, Divider, Empty } from 'antd';
import { MinusCircleOutlined, PlusOutlined } from '@ant-design/icons';
import { useNavigate } from 'react-router-dom';
import { createGuide } from '../../../services/api';
import type { IGuideCreateRequest, ICertificate } from '../../../types/guide.types';
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      const certificateData = (values.certificate || []).map((cert: any) => ({
        ...
        expiryDate: cert.expiryDate ? cert.expiryDate.format('YYYY-MM-DD') : undefined,
        documentUrl: cert.documentUrl,
      }));

      const payload: IGuideCreateRequest = {
        name: values.name,
        ...
        languages: values.languages || ['Vietnamese'],
        certificate: certificateData,
        group_type: values.group_type || 'domestic',
        health_status: values.health_status || 'healthy',
      };

      await createGuide(payload);
      message.success('Tạo HDV thành công');
      navigate('/admin/guides');
    } catch (error: any) {
      message.error(error.response?.data?.message || 'Lỗi khi tạo HDV');
      console.error(error);
    } finally {
      setLoading(false);
    }
